...
### 10.14 画像を置けるようにした（3節の visual を1種追加）

通しで観た結果、**文字が変わるだけでは毎日のニュースには足りない**という判断。
3節の `visual` に `image` を足した（`text` / `chart` / `code` / `image`）。

- `shots: [...]` に複数枚置くと**字幕のチャンク境界で切り替わる**。
  切り替え時刻は音声の実測値から来るので、絵が喋りの区切りで変わる
  （等間隔で割ると喋りの途中で変わって落ち着かない）
- 枚数の目安は「セクションの秒数 ÷ 2」。1セクション1枚では68秒で視覚が
  4回しか変わらず、`maxVisualStillSec` を字幕だけで満たしている状態になる
- ファイルは `public/shots/`。lint（`shot-missing`）が存在を確認する
- 切り替えはハードカット。クロスフェードにすると「動いている」感じが薄れる

副作用で**字幕とテロップが重なる**問題が出た（画像の分だけ内容が下に伸びた）。
`subtitleBandHeight()` を `tokens.ts` に置き、セクションの `SafeFrame` が
その分を空けるようにした。

生成AI動画（Veo / Kling）は引き続き非ゴール。画像についても、抽象的な生成画像より
